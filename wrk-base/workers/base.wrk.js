...
class WrkBase extends BfxWrkBase {
  init () {
    ...
    let storeDir = (this.ctx.env === 'test' && this.ctx.tmpdir)
      ? `${this.ctx.tmpdir}/store/${this.ctx.rack}`
      : `store/${this.ctx.rack}`

    if (this.ctx.storeDir) {
      storeDir = this.ctx.storeDir
    }
  ...
  _start (cb) {
    async.series([
      ...
      async () => {
        ...
        if (typeof this.registerRpc === 'function') {
          await this.registerRpc(rpcServer)
        }
