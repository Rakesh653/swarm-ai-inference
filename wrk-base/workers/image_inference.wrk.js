...
class ImageInferenceWorker extends WrkBase {
  init () {
    super.init()

    this.workerId = `image-${process.pid}`
    this.models = {
      embedding: embedding.infer
    }
  }
  ...
  _setupDiscovery () {
    ...
    this.logger.info({ rpcPublicKey, capabilities }, 'image worker rpc ready')
...
module.exports = ImageInferenceWorker
